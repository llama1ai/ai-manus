...
def test_file_exists_true(client, sample_text_file):
    """Test file exists check for existing file"""
    # First upload a file
    upload_url = f"{BASE_URL}/files"
    with open(sample_text_file, 'rb') as f:
        files = {'file': ('exists_test.txt', f, 'text/plain')}
        upload_response = client.post(upload_url, files=files)
    
    logger.info(f"Upload for exists test response: {upload_response.status_code} - {upload_response.text}")
    file_id = upload_response.json()['data']['file_id']
    
    # Check if file exists
    exists_url = f"{BASE_URL}/files/{file_id}/exists"
    response = client.get(exists_url)
    
    logger.info(f"File exists response: {response.status_code} - {response.text}")
    assert response.status_code == 200
    data = response.json()
    assert data['code'] == 0
...
def test_file_exists_false(client):
    ...
    fake_file_id = "507f1f77bcf86cd799439011"  # Valid ObjectId format
    url = f"{BASE_URL}/files/{fake_file_id}/exists"
    response = client.get(url)
    
    logger.info(f"File exists false response: {response.status_code} - {response.text}")
    assert response.status_code == 200
    data = response.json()
    assert data['code'] == 0
...
def test_list_files_default(client, sample_text_file):
    ...
    for i in range(3):
        with open(sample_text_file, 'rb') as f:
            files = {'file': (f'list_test_{i}.txt', f, 'text/plain')}
            upload_response = client.post(upload_url, files=files)
            file_ids.append(upload_response.json()['data']['file_id'])
    
    # List files
    list_url = f"{BASE_URL}/files"
    response = client.get(list_url)
    
    logger.info(f"List files default response: {response.status_code} - {response.text}")
    assert response.status_code == 200
    data = response.json()
    assert data['code'] == 0
    ...
    assert len(data['data']['files']) >= 3  # At least our uploaded files
...
def test_list_files_with_pagination(client):
    ...
    logger.info(f"List files with pagination response: {response.status_code} - {response.text}")
    assert response.status_code == 200
    data = response.json()
    assert data['code'] == 0
...
def test_list_files_with_filename_filter(client, sample_text_file):
    """Test listing files with filename filter"""
    # Upload a file with specific name
    upload_url = f"{BASE_URL}/files"
    with open(sample_text_file, 'rb') as f:
        files = {'file': ('filter_test_unique.txt', f, 'text/plain')}
        upload_response = client.post(upload_url, files=files)
    
    # List files with filename filter
    list_url = f"{BASE_URL}/files"
    params = {'filename': 'filter_test_unique'}
    response = client.get(list_url, params=params)
    
    logger.info(f"List files with filename filter response: {response.status_code} - {response.text}")
    assert response.status_code == 200
    data = response.json()
    assert data['code'] == 0
    # Should find our uploaded file
    found_files = [f for f in data['data']['files'] if 'filter_test_unique' in f['filename']]
    assert len(found_files) >= 1


def test_list_files_with_content_type_filter(client, sample_text_file):
    """Test listing files with content type filter"""
    # Upload a file
    upload_url = f"{BASE_URL}/files"
    with open(sample_text_file, 'rb') as f:
        files = {'file': ('content_type_test.txt', f, 'text/plain')}
        upload_response = client.post(upload_url, files=files)
    
    # List files with content type filter
    list_url = f"{BASE_URL}/files"
    params = {'content_type': 'text/plain'}
    response = client.get(list_url, params=params)
    
    logger.info(f"List files with content type filter response: {response.status_code} - {response.text}")
    assert response.status_code == 200
    data = response.json()
    assert data['code'] == 0
    # All returned files should have text/plain content type
